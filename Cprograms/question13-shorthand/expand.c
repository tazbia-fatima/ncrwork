...
int main() {
	char *arr_str=NULL, *resultant_arr = NULL;
...
char* expand(char *orig_arr, char *resultant) {
	...
	char low_lim, up_lim;
	int j = 0, length = 0;
	length = strlen(orig_arr);
	if (orig_arr[0] == '-' || orig_arr[length - 1] == '-') {
		printf("Invalid use of shorthand operator.");
		return;
	}
	for (int i = 0, j = 0; orig_arr[i] != '\0'; j++, i++) {
		...
		if ((orig_arr[i] == '-') && (orig_arr[i + 1] != '-')) {
			...
			if (up_lim < low_lim) {
				printf("Invalid shorthand"); //example h-a &capital small & a-3 etc
				getchar(); 
				free(resultant);
			}
			int diff = (int)up_lim - (int)low_lim - 1;
			printf("%d", diff); //case for diff =0
			//resultant = realloc(resultant, sizeof(char)*(diff));
			char c = low_lim;
			for (int k = 0; k<diff; k++) { //alphaiterator example
				c = c + 1;
				printf("\nvalue of c %c", c);
				resultant[j] = c;
